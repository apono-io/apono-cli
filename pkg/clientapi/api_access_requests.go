...
type ApiDryRunAccessRequestByIdRequest struct {
	ctx        context.Context
	ApiService *AccessRequestsAPIService
	id         string
}

func (r ApiDryRunAccessRequestByIdRequest) Execute() (*DryRunClientResponse, *http.Response, error) {
	return r.ApiService.DryRunAccessRequestByIdExecute(r)
}

/*
DryRunAccessRequestById Dry run validation for an existing access request

	@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@param id
	@return ApiDryRunAccessRequestByIdRequest
*/
func (a *AccessRequestsAPIService) DryRunAccessRequestById(ctx context.Context, id string) ApiDryRunAccessRequestByIdRequest {
	return ApiDryRunAccessRequestByIdRequest{
		ApiService: a,
		ctx:        ctx,
		id:         id,
...
func (a *AccessRequestsAPIService) DryRunAccessRequestByIdExecute(r ApiDryRunAccessRequestByIdRequest) (*DryRunClientResponse, *http.Response, error) {
	var (
		localVarHTTPMethod  = http.MethodPost
		localVarPostBody    interface{}
		formFiles           []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "AccessRequestsAPIService.DryRunAccessRequestById")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/client/v1/access-requests/{id}/dry-run"
	localVarPath = strings.Replace(localVarPath, "{"+"id"+"}", url.PathEscape(parameterValueToString(r.id, "id")), -1)
